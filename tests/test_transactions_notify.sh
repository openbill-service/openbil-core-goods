...
./tests/assert_result_include.sh "listen OPENBILL_TRANSACTIONS; insert into OPENBILL_TRANSACTIONS (owner_id, id, amount_cents, amount_currency, from_account_id, to_account_id, key, details) values ($OWNER_UUID, '$TRANSACTION_UUID', 100, 'USD', $ACCOUNT1_UUID, $ACCOUNT2_UUID, 'gid://order1', 'test')" "Asynchronous notification \"openbill_transactions\" with payload \"$TRANSACTION_UUID\" received from server"
